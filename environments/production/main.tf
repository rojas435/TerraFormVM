...
module "security" {
  ...
  # Production environment has very restrictive SSH access
  allowed_ssh_sources = var.allowed_ssh_sources

  # Custom security rules for production
...
module "compute" {
  ...
  # Enhanced OS disk configuration for production
  os_disk_config = {
    caching              = "ReadWrite"
    storage_account_type = "Premium_LRS"  # Premium SSD for production
    disk_size_gb         = 128
  }
